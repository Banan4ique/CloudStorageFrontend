    ...
    <!-- Модальное окно для переименования -->
        <div v-if="showRenameModal" class="modal-overlay">
            <div class="modal">
                ...
                <input v-model="renameNewName" ref="renameInput" />
...
<script lang="ts">
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const renameFileObj = ref<any>(null);
            const renameInput = ref<HTMLInputElement | null>(null);

            watch(showRenameModal, (newVal) => {
                if (!newVal) {
            ...
            const fileChangeName = (file: any) => {
                ...
                renameNewName.value = getFileNameWithoutExt(file);
            ...
            const confirmRename = () => {
                if (!renameFileObj.value || !renameNewName.value.trim()) {
                    showRenameModal.value = false;
                    return;
                }

                const file = renameFileObj.value;
                const newFilename = `${renameNewName.value.trim()}.${file.ext}`;

                store.dispatch('addNotification', {
                    text: `Переименование файла "${file.filename}"`,
                    status: 'loading'
                }).then(notification => {
                    updateFile(file.filename, {filename: newFilename})
                        .then(res => {
                            file.filename = newFilename;
                            notification.status = 'success';
                            notification.text = `Файл переименован в "${newFilename}"`;

                            // Закрываем модальное окно при успехе сразу
                            showRenameModal.value = false;
                            renameFileObj.value = null;
                            renameNewName.value = '';
                        })
                        .catch(res => {
                            file.error = true;
                            notification.status = 'error';
                            notification.text = `Не удалось переименовать файл`;

                            // Закрываем модальное окно при ошибке тоже сразу
                            showRenameModal.value = false;
                            renameFileObj.value = null;
                            renameNewName.value = '';
